...
CREATE INDEX "tags_name" ON tags USING BTREE ("name") WHERE LENGTH("name") > 64 AND LENGTH("name") < 128;
--- Tag Value Index (under 128 chracters)
CREATE INDEX "tags_value" ON tags USING BTREE ("value") WHERE LENGTH("value") > 64; AND LENGTH("value") < 128;
--- Tag Name, Value Index (under 128 chracters)
CREATE INDEX "tags_name_value" ON tags USING BTREE ("name", "value") WHERE LENGTH("name") > 64 AND LENGTH("name") < 128 AND LENGTH("value") > 64 AND LENGTH("value") < 128;
--- Tag Transaction Id, Name Index (under 128 chracters)
CREATE INDEX "tags_tx_id_name" ON tags USING BTREE ("tx_id", "name") WHERE LENGTH("name") > 64 LENGTH("name") < 128;
